...
def mv_to_adc(x, range_, bitness=16):
    v_ranges = [10, 20, 50, 100, 200, 500, 1_000, 2_000, 5_000, 10_000, 20_000]
    return int((x * (2 ** (bitness - 1) - 1)) / v_ranges[range_])
...
def handle_request(c, req, b_sock, b_addr):
    ...
    recovered_timeshift, freq_A, period_A, freq_B, period_B = phase_shift(
        a_wave, b_wave
    )
    print("Recovered offset: {} degrees".format(recovered_timeshift))

    A_pk, A_vly, A_clipped, A_flipped, A_mid, _ = peaks_valleys(
        a_wave.y, period_A / a_wave.dt.s
    )
    A_bounces = bounces(A_pk, A_vly, A_clipped)
    A_duty = duty_cycle(period_A, A_pk[1]["widths"], a_wave.dt.s)

    B_pk, B_vly, B_clipped, B_flipped, B_mid, _ = peaks_valleys(
        b_wave.y, period_B / b_wave.dt.s
    )
...
class StreamingDevice:
    ...
    def gather(self):
        ...
        return (
            adc_to_mv([*pretriggerA, *adc_valuesA], self.potential_range),
            adc_to_mv([*pretriggerB, *adc_valuesB], self.potential_range),
            start_at,
        )
...
# Start device
picoDevice = StreamingDevice( potential_range=ps2000.PS2000_VOLTAGE_RANGE["PS2000_20V"] )
...
while True:
    ...
    print(
        f"[+] Connection established from: {partner[0]}:{partner[1]} | Socket: {c_sock}"
    )
